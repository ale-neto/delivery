...
class Owner extends MY_Controller
{
    ...
    | -----------------------------------------------------
    |
    | MODULE:           Owner CONTROLLER
    | -----------------------------------------------------
    | This is Owner module controller file.
    | -----------------------------------------------------
     **/
    ...
    function index()
    {
        ...
        $data = $this->owner_model->get_modules_count();
        $this->data['modules_count']  =  (object) $data;
        
        $this->data['activemenu']     = ACTIVE_OWNER;
        
        $this->data['message']         = $this->session->flashdata('message');
        $this->data['pagetitle']       = get_languageword('owner_dashboard');
        $this->data['content']         = PAGE_OWNER_DASHBOARD;
        $this->_render_page(TEMPLATE_OWNER, $this->data);
    }
    ...
    function profile()
    {
        ...
        if (isset($_POST['update_profile'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                redirect(URL_OWNER_PROFILE, REFRESH);
            }
        }
        ...
        $this->data['content']          = PAGE_OWNER_RROFILE;
        $this->_render_page(TEMPLATE_OWNER, $this->data);    
    }
    ...
    function change_password()
    {
        if (isset($_POST['change_pwd'])) {
            ...
            if (DEMO) {
                $this->prepare_flashmessage(get_languageword('CRUD_operations_disabled_in_DEMO_version'), 2);
                redirect(URL_OWNER_CHANGE_PASSWORD);
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($change) {
                    ...
                    redirect(URL_OWNER_CHANGE_PASSWORD, REFRESH);
                }
            }
        }
