...
class Ion_auth
{
			...
			if ($user)
			{
				...
				else
				{
					...
					if(!empty($email_template))
					{
						...
						$content 	= str_replace("___RESET_YOUR_PASSWORD___", '<a href="'.URL_AUTH_RESET_PASSWORD.DS.$data['forgotten_password_code'].'">'.get_languageword('Reset_your_Password').'</a>',$content);
						...
						$sub = ucwords(str_replace('_',' ',$email_template->subject)).' '.$this->config->item('site_settings')->site_title;
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array(),$fb = FALSE) //need to test email activation
	{
		...
		else
		{
			...
			$data = array(
				...
				'refer_by_code'=>$user->refer_by_code	
			);
			...
			else
			{
				...
				if($fb)
				{
					...
					if(!empty($email_template))
					{
						...
						$subject = str_replace('_',' ',$email_template->subject).' '.$this->config->item('site_settings')->site_title;
				...
				else
				{
					...
					$email_template = $this->db->query("SELECT * FROM ".TBL_PREFIX.TBL_EMAIL_TEMPLATES." WHERE subject = 'registration' ")->result();
					if(!empty($email_template))
					{
						$email_template = $email_template[0];
						$content 	= $email_template->email_template;
						...
						if($group[0]->id==3)
						{
							$content 	= str_replace("__SITE_TITLE__", $this->config->item('site_settings')->site_title.'  as a Kitchen Manager',$content);
						}
						else if($group[0]->id==4)
						{
							$content 	= str_replace("__SITE_TITLE__", $this->config->item('site_settings')->site_title.'  as a Delivery Manager',$content);
						}
						else
						{
							$content 	= str_replace("__SITE_TITLE__", $this->config->item('site_settings')->site_title,$content);
						...
						if($data['refer_by_code'] != '')
						{
							$content 	= str_replace("__ACCOUNT_ACTIVATOIN_LINK__", anchor(SITEURL2.'/auth/activate/'.$id.'/'.$activation_code.'/'.$data['refer_by_code'],'Activate Your Account'),$content);
						}
						else
						{
							$content 	= str_replace("__ACCOUNT_ACTIVATOIN_LINK__", anchor(SITEURL2.'/auth/activate/'.$id.'/'.$activation_code,'Activate Your Account'),$content);
						}							
	...
	public function is_owner($id=false)
	{
		$this->ion_auth_model->trigger_events('is_owner');

		$owner_group = $this->config->item('owner_group', 'ion_auth');

		return $this->in_group($owner_group, $id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_member($id=false) //is_user also same
	{
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_kitchen_manager($id=false) //is_user also same
	{
		$this->ion_auth_model->trigger_events('kitchen_manager_group');
		$kitchen_manager_group = $this->config->item('kitchen_manager_group', 'ion_auth');		
		return $this->in_group($kitchen_manager_group, $id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_delivery_manager($id=false)//is_user also same
	{
		$this->ion_auth_model->trigger_events('delivery_manager_group');
		$delivery_manager_group = $this->config->item('delivery_manager_group', 'ion_auth');		
		return $this->in_group($delivery_manager_group, $id);
	}	
	/**
	 * in_group
	 *
